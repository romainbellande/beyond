...
impl Resource {
  ...
  pub fn rand_list() -> Vec<Resource> {
    ...
    let nb_resources_weight: Vec<u32> = nb_resources_weight_reversed.iter().copied().rev().collect();
    ...
    for _ in 0..max_resources {
      ...
      let resource_config_to_add: ResourceConfig = resource_config_list[resource_index].clone();
